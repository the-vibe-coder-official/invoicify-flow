...
Nun aktualisiere ich auch die `create-checkout` Edge Function:

<lov-write file_path="supabase/functions/create-checkout/index.ts">
import { serve } from "https://deno.land/std@0.190.0/http/server.ts";
import Stripe from "https://esm.sh/stripe@14.21.0";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.45.0";
...
serve(async (req) => {
  ...
  try {
    ...
    const priceData = {
      Pro: {
        currency: "eur",
        product_data: { name: "Pro Plan - 20 Rechnungen pro Monat" },
        unit_amount: 999, // €9.99
        recurring: { interval: "month" },
      },
      Unlimited: {
        currency: "eur",
        product_data: { name: "Unlimited Plan - Unbegrenzte Rechnungen" },
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: priceData[tier as keyof typeof priceData],
          quantity: 1,
        },
      ],
      mode: "subscription",
